...
//  Blocstagram
//
//  Created by Brian Doore on 10/22/14.
...
- (void) addFiltersToQueue {
    ...
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        ...
        if (instantFilter) {
            ...
            if (bloomFilter) {
                [bloomFilter setValue:result forKeyPath:kCIInputImageKey];
                //[vignetteFilter setValue:[NSNumber numberWithFloat:0.1] forKey:@"inputIntensity"];
                //[vignetteFilter setValue:[NSNumber numberWithFloat:1.0] forKey:@"inputRadius"];
                result = bloomFilter.outputImage;
            }
            
            [self addCIImageToCollectionView:result withFilterTitle:NSLocalizedString(@"Bloom/Instant", @"Bloom/Instant Filter")];
        }
    }];
